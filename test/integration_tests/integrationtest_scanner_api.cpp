...
TEST_F(ScannerAPITests, shouldReturnInvalidFutureWhenStartIsCalledSecondTime)
{
  ...
  scanner_mock.startListeningForControlMsg();
  const auto start_future = scanner.start();
  EXPECT_TRUE(start_future.valid()) << "First call too Scanner::start() should return valid std::future object";
  EXPECT_FALSE(scanner.start().valid()) << "Second call too Scanner::start() should return invalid std::future object";
...
TEST_F(ScannerAPITests, shouldReturnInvalidFutureWhenStopIsCalledSecondTime)
{
  ...
  EXPECT_TRUE(stop_future.valid()) << "First call too Scanner::stop() should return valid std::future object";
  EXPECT_FALSE(scanner.stop().valid()) << "Second call too Scanner::stop() should return invalid std::future object";
