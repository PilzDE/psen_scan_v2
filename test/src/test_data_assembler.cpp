...
int64_t secToNSec(const double& sec)
{
  double nsec{ sec * 1000000000.0 };
  return static_cast<int64_t>(nsec >= 0 ? nsec + 0.5 : nsec - 0.5);
}

std::unique_ptr<TestData> TestDataAssembler::assemble(const ScannerConfiguration& scanner_config,
                                                      ...
                                                      const uint16_t udp_port)
{
  ...
  UdpData udp_data;
  UdpDataReader::read(udp_data_filename, udp_port, udp_data);
...
void TestDataAssembler::runScanner(const ScannerConfiguration& scanner_config,
                                   const int64_t scanner_run_duration_sec,
...
void TestDataAssembler::addUdpData(TestData& test_data, const UdpData& udp_data)
{
...
void TestDataAssembler::removeIncompleteData(TestData& test_data)
{
  test_data.erase(
...
void TestDataAssembler::sortWithRespectToScanCounter(TestData& test_data)
{
...
void TestDataAssembler::extractDataFromScan(TestData& test_data, const LaserScan& scan)
{
