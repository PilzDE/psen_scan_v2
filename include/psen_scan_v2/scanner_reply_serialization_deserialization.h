...
Message deserialize(const T& data)
{
  ...
  const auto crc = raw_processing::read<uint32_t>(is);
  const auto reserved = raw_processing::read<uint32_t>(is);
  const auto opcode = raw_processing::read<uint32_t>(is);
  const auto res_code = raw_processing::read<uint32_t>(is);
