...
using TestScanRange = configuration::ScanRangeTemplated<MIN_ALLOWED_ANGLE.value(), MAX_ALLOWED_ANGLE.value()>;

TEST(ScanRangeTest, testCtorCallForCoverage)
{
  std::unique_ptr<configuration::ScanRange> ptr{ new configuration::ScanRange(MIN_ALLOWED_ANGLE, MAX_ALLOWED_ANGLE) };
}
