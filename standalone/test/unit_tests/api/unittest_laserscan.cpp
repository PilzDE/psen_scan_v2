...
{
static const TenthOfDegree DEFAULT_RESOLUTION{ 1 };
...
{
  EXPECT_THROW(api::LaserScan laser_scan(TenthOfDegree(0), DEFAULT_START_ANGLE, DEFAULT_END_ANGLE),
               std::invalid_argument);
}

TEST(LaserScanTests, testOutOfRangeResolution)
{
  EXPECT_THROW(api::LaserScan laser_scan(TenthOfDegree(10000), DEFAULT_START_ANGLE, DEFAULT_END_ANGLE),
               std::invalid_argument);
}
...
{
  ASSERT_NO_THROW(api::LaserScan(DEFAULT_RESOLUTION, TenthOfDegree(1000u), TenthOfDegree(1000u)););
}
