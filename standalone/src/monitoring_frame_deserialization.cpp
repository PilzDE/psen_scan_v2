  ...
  while (!end_of_frame)
  {
    ...
    switch (static_cast<additional_field::HeaderID>(additional_header.id()))
    {
      case additional_field::HeaderID::scan_counter:
        ...
        raw_processing::readTyped<uint32_t, boost::optional<uint32_t>>(is, msg.scan_counter_);
        break;
...
FixedFields readFixedFields(std::istringstream& is)
{
  ...
  raw_processing::readTyped<int16_t, TenthOfDegree>(is, from_theta);
  raw_processing::readTyped<int16_t, TenthOfDegree>(is, resolution);
